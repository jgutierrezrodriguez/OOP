...
void StarFighter::addPiece(Piece & piece){
    parts[numParts]=&piece;
    
    numParts++;
}

void StarFighter::deletePiece(int numPiece){
    delete parts[numPiece];
}

float StarFighter::calculateWeight(){
    float total=0;
    for(int i=0;i<numParts;i++){
        total=total+parts[i]->getWeight();
    }
