...
void Mutant::fromCSV(string &inputStr){
    stringstream ss;
    ss << inputStr;
...
Mutant& Mutant::operator=(const Mutant& orig) {
    if (this != &orig) {
        ...
        numPowers=orig.numPowers;
        
        
        for(int i =0; i<orig.numPowers;i++){
            powers[i]=orig.powers[i];
        }
    }
...
bool Mutant::operator <(Mutant& orig){
...
void Mutant::addPsychicalPower(string name, string description, string affectsTo, float destructiveCapacity,float lucidity){
    
    if (numPowers == MAX_POWERS) {
        throw std::length_error("Mutant::addPsychicalPower: A mutant can't have more than 10 powers.");
    }
    
    powers[numPowers]=new PsychicalPower(name, description, affectsTo, destructiveCapacity,lucidity);
    numPowers++;
}
